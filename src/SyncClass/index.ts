 ...
 * @property {Function} getMiniProfileId {@link steamUser.getMiniProfileId} - Retrieves the mini profile id.
...
export class steamUser {
   ...
   * @method getUserInfo
   * @param {string} x - (optional) The string to set as the 'steamHtml' property.(if use without class)
   * @param {string} miniProfileHtml - (optional) The string to set as the 'miniProfileHtml' property.
   * @return {userInfo} The user information object.
   */
  getUserInfo(x?: string, miniProfileHtml?: string): userInfo {
    try {
      const $def = () => {
        if (x) {
          if (this !== undefined) {
      ...
      const backgroundDef = (): backgroundObj | null => {
        if (miniProfileHtml) {
      ...
      const background: backgroundObj | null = backgroundDef();
   ...
   */
  getBasicProfile(x?: string, miniProfileHtml?: string): basicUserInfo {
    try {
      if (!x && !this) {
        throw new Error("Steam html not found");
      }
      ...
      const { nickname, avatar, avatarFrame, level, mainBadge, background } =
        steam.getUserInfo("", miniProfileHtml);
      const basicProfile = {
        status,
        userInfo: {
          nickname,
  ...
  getMiniProfileId(x?: string): number {
    try {
      const $def = () => {
        if (x) {
          if (this !== undefined) {
      ...
      const miniProfileId = parseInt(
        $(".playerAvatar").attr("data-miniprofile")
      );
